...
class MaskFormerDecoder(nn.Module):
    def __init__(
        ...
        dim: int,
        decoder_layer_type: str = "hepattn",
        ...
        elif decoder_layer_type == "hepformer":
        ...
        # else:
        self.inital_q = nn.Parameter(torch.empty((num_queries, self.dim)))
    ...
    def forward(self, x: dict[str, Tensor], input_names: list[str]) -> tuple[dict[str, Tensor], dict[str, dict]]:
        ...
        preds = {"queries": x["query_embed"], "x": x["hit_embed"], **self.get_preds(x["query_embed"], x["hit_embed"])}
        if self.aux_loss:
