...
class HitFilterTask(Task):
    def __init__(
        ...
        if self.unified_decoding:
            self.input_objects = ["key_embed"]
        else:
            self.input_objects = [f"{input_object}_embed"]
        self.net = Dense(dim, 1)

    def forward(self, x: dict[str, Tensor]) -> dict[str, Tensor]:
        x_logit = self.net(x["key_embed"]) if self.unified_decoding else self.net(x[f"{self.input_object}_embed"])
        return {"key_logit": x_logit.squeeze(-1)} if self.unified_decoding else {f"{self.input_object}_logit": x_logit.squeeze(-1)}

    def predict(self, outputs: dict[str, Tensor]) -> dict[str, Tensor]:
        if self.unified_decoding:
            return {f"key_{self.target_field}": outputs["key_logit"].sigmoid() >= self.threshold}
    ...
    def loss(self, outputs: dict[str, Tensor], targets: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        output = outputs["key_logit"] if self.unified_decoding else outputs[f"{self.input_object}_logit"]
        target = (
            targets[f"hit_{self.target_field}"].type_as(output)
            if self.unified_decoding
            else targets[f"{self.input_object}_{self.target_field}"].type_as(output)
        )
        ...
        if self.loss_fn == "bce":
            ...
            if self.unified_decoding:
                return {f"key_{self.loss_fn}": loss}
    ...
    def key_mask(self, outputs: dict[str, Tensor], threshold: float = 0.1) -> dict[str, Tensor]:
        if not self.mask_keys:
            return {}
        if self.unified_decoding:
            return {"key": outputs["key_logit"].detach().sigmoid() >= threshold}
