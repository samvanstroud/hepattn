...
class PlotParticleHelper:
    ...
    def plot_residuals(self, pt_relative=False, log_y=False, qs=None):
        """Args:
        ...
        for name in self.perf.network_names:
            ref_dict, comp_dict, _, _ = self.perf.data[name]["matched_particles"]
    ...
    def plot_residuals_neutrals(self, pt_relative=False, log_y=False, qs=None, separate_figures=False):
        """Args:
        data_dicts: {name: (ref_dict, comp_dict), ...}
            ref_dict: {'pt': [], 'eta': [], 'phi': [], 'class': []} # flat_arrays.
        """
        ...
        for name in self.perf.network_names:
            ref_dict, comp_dict, ref_dict_unmatched, _ = self.perf.data[name]["matched_particles"]
            for var in ["pt", "eta", "phi"]:
                residual_dict[name][var] = comp_dict[var] - ref_dict[var]
                if var == "pt" and pt_relative:
                    residual_dict[name][var] /= ref_dict["pt"]
                ...
                abs_max_dict["Photon"][var] = max(
                    abs_max_dict["Photon"][var], np.percentile(np.abs(residual_dict[name][var][~mask]), qs["Photon"][var])
                )
    ...
    def plot_eff_fr_purity(self, colors=None):
        ...
        for name in self.perf.network_names:
            ref_dict, comp_dict, ref_dict_unmatched, comp_dict_unmatched = self.perf.data[name]["matched_particles"]
            truth_pt_matched = ref_dict["pt"]
            truth_class_matched = ref_dict["class"]
            pflow_pt_matched = comp_dict["pt"]
            pflow_class_matched = comp_dict["class"]
            truth_pt_unmatched = ref_dict_unmatched["pt"]
            truth_class_unmatched = ref_dict_unmatched["class"]
            pflow_pt_unmatched = comp_dict_unmatched["pt"]
            pflow_class_unmatched = comp_dict_unmatched["class"]
            ...
            for cl_name, cl_dict in eff_fr_dict.items():
                ax1.hist(
                    ...
                    bins=pt_bins,
                    weights=cl_dict["eff"],
                    label=f"{cl_name} ({self.labels[name]})",
                    linestyle=linestyle,
                    histtype="step",
                    linewidth=1,
