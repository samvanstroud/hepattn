...
def build_strided_sliding_window_blockmask(
    *,
    ...
    indices, num_blocks = compute_intersecting_blocks(q_blocks, kv_blocks, stride, half_window, block_size, q_len, kv_len, device, wrap)
...
def compute_intersecting_blocks(
    ...
    for qb in range(q_blocks):
        ...
        compute_block_indices_for_query_block = compute_block_indices_for_query_block_wrap if wrap else compute_block_indices_for_query_block_no_wrap
        num_blocks[qb], indices = compute_block_indices_for_query_block(lo_tok, hi_tok, qb, kv_len, block_size, indices, kv_blocks, device)
    return indices, num_blocks


def compute_block_indices_for_query_block_no_wrap(lo_tok, hi_tok, qb, kv_len, block_size, indices, kv_blocks, device):  # noqa: ARG001
...
def compute_block_indices_for_query_block_wrap(lo_tok, hi_tok, qb, kv_len, block_size, indices, kv_blocks, device):
