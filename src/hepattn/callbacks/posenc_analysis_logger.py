...
def create_pos_enc_visualizations(
    ...
    if hit_phi_sorted is not None or query_phi_sorted is not None:
        ...
        if query_phi_sorted is not None:
            ...
            axes[0, 0].plot(range(len(query_phi_sorted)), query_phi_sorted.numpy(), color="skyblue", linewidth=2, marker="o", markersize=3)
        ...
        if hit_phi_sorted is not None:
            ...
            axes[1, 0].plot(range(len(hit_phi_sorted)), hit_phi_sorted.numpy(), color="lightcoral", linewidth=2, marker="o", markersize=3)
        ...
        if hit_phi_sorted is not None and query_phi_sorted is not None:
            ...
            axes[0, 1].plot(
                range(len(query_phi_sorted)), query_phi_sorted.numpy(), color="skyblue", linewidth=2, label="Queries", marker="o", markersize=3
            )
            axes[0, 1].plot(
                range(len(hit_phi_sorted)), hit_phi_sorted.numpy(), color="lightcoral", linewidth=2, label="Hits", marker="o", markersize=3
            )
            ...
            axes[1, 1].bar(["Query Range", "Hit Range"], [query_range.item(), hit_range.item()], color=["skyblue", "lightcoral"])
            ...
            axes[1, 1].text(
                0.02,
                0.98,
                f"Query: [{query_phi_sorted.min():.3f}, {query_phi_sorted.max():.3f}]\nHit: [{hit_phi_sorted.min():.3f}, {hit_phi_sorted.max():.3f}]",
