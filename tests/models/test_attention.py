...
def test_attention_consistency(batch_size, dim, num_heads, bias, q_len, kv_len, attn_masking, kv_masking, attn_type):
    ...
    if attn_type == "flash-varlen":
        if kv_mask is None:
            kv_mask = torch.full((batch_size, kv.shape[-2]), True, dtype=torch.bool, device="cuda")
...
def test_self_attention(batch_size, seq_len, dim, num_heads, bias, attn_type):
    ...
    # Initialize attention layers
    attn_torch = Attention(dim=dim, num_heads=num_heads, bias=bias, attn_type="torch").cuda().half()
    attn_test = Attention(dim=dim, num_heads=num_heads, bias=bias, attn_type=attn_type).cuda().half()
    ...
    varlen_kwargs = None
    if attn_type == "flash-varlen":
        kv_mask = torch.full((batch_size, seq_len), True, dtype=torch.bool, device="cuda")
...
def test_attention_change_backend(attn_type, attn_type_new):
    ...
    varlen_kwargs = None
    if attn_type == "flash-varlen":
        kv_mask = torch.full((1, kv.shape[-2]), True, dtype=torch.bool, device="cuda")
